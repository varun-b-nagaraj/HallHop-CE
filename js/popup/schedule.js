...
export async function getCurrentClassInfo(username, password, customNow = null, studentId = null) {
    ...
    try {
      ...
      if (isMidDayPeriod && currentClass.room) {
        ...
        // Extract building number (first 1-3 digits)
        const buildingNum = parseInt(roomStr.match(/^\d{1,3}/)?.[0] || '0');
        
        debug(`Processing room ${roomStr}, extracted building number: ${buildingNum}`);
        
        // A Lunch: Buildings 200s, 700s, 900s, 1000s
        if ([200, 700, 900, 1000].some(building => 
            buildingNum >= building && buildingNum < building + 100)) {
          lunchDesignation = "A";
        }
        // B Lunch: Buildings 100s, 1300s
        else if ([100, 1300].some(building => 
            buildingNum >= building && buildingNum < building + 100)) {
          lunchDesignation = "B";
        }
        // C Lunch: Buildings 600s, 2200s, 2300s
        else if ([600, 2200, 2300].some(building => 
            buildingNum >= building && buildingNum < building + 100)) {
          lunchDesignation = "C";
        }
        // D Lunch: Buildings 400s, 500s, 1100s, 1400s/2400s, 1500s/2500s
        else if ([400, 500, 1100, 1400, 2400, 1500, 2500].some(building => 
            buildingNum >= building && buildingNum < building + 100)) {
          lunchDesignation = "D";
        }
        else {
          ...
          lunchDesignation = "D";
          debug(`No lunch designation found for room ${roomStr}, defaulting to D lunch`);
        }
        
        debug(`Assigned ${lunchDesignation} lunch based on room ${roomStr} (building ${buildingNum})`);
      }
